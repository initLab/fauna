...
class UserRole < ApplicationRecord
  ...
  scope :expired, lambda {
    unscope(where: %i[start_time end_time]).where('(end_time is not null and end_time < ?)', Time.current)
  }
  scope :future, lambda {
    unscope(where: %i[start_time end_time]).where(
      '(start_time is not null and start_time > ? and (end_time is null or end_time > ?))',
      Time.current,
      Time.current
    )
  }
