...
describe User, type: :model do
  ...
  describe '::find_for_database_authentication' do
    ...
    it 'returns nil when the passed login does not exist' do
      expect(described_class.find_for_database_authentication(login: 'randomuser')).to be_nil
    end
    ...
    it 'returns the user despite of case differences in her login' do
      expect(described_class.find_for_database_authentication(login: " #{user.email.upcase}")).to eq user
      expect(described_class.find_for_database_authentication(login: " #{user.username.upcase}")).to eq user
    end
    ...
    context 'when there are users with no username' do
      ...
      it 'returns the legacy user when passed her email' do
        expect(described_class.find_for_database_authentication(login: legacy_users.first.email))
          .to eq legacy_users.first
      end

      it 'returns nil when the login is nil' do
        expect(described_class.find_for_database_authentication(login: nil)).to be_nil
      end

      it 'returns nil when the login is omitted' do
        expect(described_class.find_for_database_authentication({})).to be_nil
      end
  ...
  describe 'url' do
    ...
    it 'can be nil' do
      expect(build(:user, url: nil)).not_to have_error_on :url
      expect(build(:user, url: '')).not_to have_error_on :url
    end

    it 'can be an HTTP URL' do
      expect(build(:user, url: 'http://example.com')).not_to have_error_on :url
    end

    it 'can be an HTTPS URL' do
      expect(build(:user, url: 'https://example.com')).not_to have_error_on :url
    end
  ...
  describe 'twitter handle' do
    ...
    it 'can be nil' do
      expect(build(:user, twitter: nil)).not_to have_error_on :twitter
    end

    it 'can be a valid handle' do
      valid_handles.each do |handle|
        expect(build(:user, twitter: handle)).not_to have_error_on :twitter
      end
    end
  ...
  describe 'github username' do
    it 'can be nil' do
      expect(build(:user, github: nil)).not_to have_error_on :github
    end

    it 'can only contain alphanumeric characters or dashes' do
      expect(build(:user, github: '!foobar')).to have_error_on :github
      expect(build(:user, github: 'foobar')).not_to have_error_on :github
      expect(build(:user, github: 'foobar-')).not_to have_error_on :github
      expect(build(:user, github: 'foobar123')).not_to have_error_on :github
    end
    ...
    it 'cannot be longer than 39 characters' do
      ...
      expect(build(:user, github: 'a' * 39)).not_to have_error_on :github
    end
  end

  describe 'jabber account' do
    it 'can be nil' do
      expect(build(:user, jabber: nil)).not_to have_error_on :jabber
    end

    it 'must contain exactly one @' do
      expect(build(:user, jabber: 'foo@bar.com')).not_to have_error_on :jabber
  ...
  describe '#pin' do
    it 'returns the pin' do
      expect(build(:user, pin: 123_456).pin).to eq 123_456
    end
  end
  ...
  describe '#pin=' do
    it 'does not set the value of encrypted_pin whenever pin is blank or nil' do
      ...
      expect { user.pin = '' }.not_to change(user, :encrypted_pin)
      expect { user.pin = nil }.not_to change(user, :encrypted_pin)
    end

    it 'sets the value of encrypted_pin to a BCrypt::Password encrypted version of pin' do
      user = build :user

      allow(BCrypt::Password).to receive(:create).and_return(BCrypt::Password.new('$2a$10$eiD5w/oMitEM1kXoFmxWQOFlLCNZTAwV/IZOGo8UMIROvg0W4iXRy'))
  ...
  describe 'pin' do
    it 'can be nil' do
      expect(build(:user, pin: nil)).not_to have_error_on :pin
    end

    it "must be a number with 6 or more digits when it's not nil" do
      expect(build(:user, pin: 123_456)).not_to have_error_on :pin
      expect(build(:user, pin: 1_234_567_890)).not_to have_error_on :pin
    ...
    it 'must check if confirmation matches' do
      expect(build(:user, pin: 123_456, pin_confirmation: 123_456)).not_to have_error_on :pin_confirmation
      expect(build(:user, pin: 123_456, pin_confirmation: 12_345)).to have_error_on :pin_confirmation
    end
  end
