...
describe SpaceApi, type: :model do
  ...
  it 'has methods for all keys in the SPACEAPI_CONFIG' do
    spaceapi_config.each_key do |key|
      expect(described_class.new).to respond_to key
    end
  end
  ...
  describe '#to_json' do
    it 'returns a json object that contains all attributes of the class' do
      expected = HashWithIndifferentAccess.new(described_class.new.serializable_hash)
  ...
  describe '#api' do
    it 'returns 0.13' do
      expect(described_class.new.api).to eq '0.13'
    end
  end
  ...
  describe '#sensors' do
    describe 'its entry with key people_now_present' do
      it 'is an array' do
        expect(described_class.new.sensors[:people_now_present]).to be_an Array
      end

      it 'contains a single member' do
        expect(described_class.new.sensors[:people_now_present].count).to eq 1
      end
      ...
      it 'contains a member that is a hash with the name of the hackerspace in its entry with a key of location' do
        ...
        expect(described_class.new.sensors[:people_now_present][0][:location]).to eq 'foobar'
      end
    end

    context 'when there are people in the lab' do
      describe 'its entry with key people_now_present' do
        it 'contains a member that is a hash with the current number of present people in its entry' do
          allow(Presence).to receive(:present_users).and_return(create_list(:user, 2))
          expect(described_class.new.sensors[:people_now_present][0][:value]).to eq 2
        end
        ...
        it 'contains a member that is a hash with the names of the present users in its entry with a key of names' do
          ...
          expect(described_class.new.sensors[:people_now_present][0][:names]).to eq(present_people.map(&:username))
        end
      end
    end
    ...
    context 'when there are not people in the lab' do
      describe 'its entry with key people_now_present' do
        it 'contains a member that is a hash with the current number of present people in its entry' do
          ...
          expect(described_class.new.sensors[:people_now_present][0][:value]).to eq 0
        end

        it 'contains a member that is a hash that does not have an entry with a key of names' do
          allow(Presence).to receive(:present_users).and_return([])
          expect(described_class.new.sensors[:people_now_present][0]).not_to have_key :names
        end
      end
    end
  end
